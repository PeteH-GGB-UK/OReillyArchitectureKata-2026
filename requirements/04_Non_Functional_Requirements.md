...
| ID / Requirement | Operating Condition | Threshold / Acceptance Criteria | Verification Method |
|---|---|---|---|
| NFR-03 — Availability | Admission during opening hours; central ride and animal-care workflows during their agreed service hours. | Each service ≥99.9% availability per calendar month within its service window, including planned interruptions. Define successful user transactions separately for each service. | Synthetic user journeys and incident records; calculate service-specific availability and report local fallback separately. |
